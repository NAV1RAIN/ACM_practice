...
//    freopen("out", "w", stdout);
    ...
    while (T--) {
        ...
        int jj = max(max(len1, len2), len3);
        BigInteger t1 = BigInteger(s1), t2 = BigInteger(s2), t3 = BigInteger(s3);
        for (int i = len1; i < jj; i++) {
            str1[i] = '0';
        }
        for (int i = len2; i < jj; i++) {
            str2[i] = '0';
        }
        for (int i = len3; i < jj; i++) {
            str3[i] = '0';
        }
        s1 = str1, s2 = str2, s3 = str3;
        BigInteger tt1 = BigInteger(s1), tt2 = BigInteger(s2), tt3 = BigInteger(s3);
        str3[jj] = '0';
        s3 = str3;
        BigInteger ttt3 = BigInteger(s3);
        if (ttt3 == (tt1 + tt2)) {
            cout << jj-len1 << ' ' << jj-len2 << ' ' << jj-len3+1 << '\n';
        } else if((tt3-tt1==t2)||(tt3-tt1>=t2*ten&&(tt3-tt1)%(t2*ten) == zero)) {
            BigInteger temp = (tt3-tt1)/t2/ten;
            cout << jj-len1 << ' ' << temp << ' ' << jj-len3 << '\n';
        } else if((tt3-tt2==t1)||(tt3-tt2>=t1*ten&&(tt3-tt2)%(t1*ten) == zero)) {
            BigInteger temp = (tt3-tt2)/t1/ten;
            cout << temp << ' ' << jj-len2 << ' ' << jj-len3 << '\n';
        } else {
            cout << -1 << '\n';
        }
    }
